...
public class DmnScalaTCKTest implements DmnTckVendorTestSuite {
	...
	private static final Logger logger = LoggerFactory.getLogger(DmnScalaTCKTest.class);
	...
	public TestSuiteContext createContext() {
		final TestRunnerContext context = new TestRunnerContext();
	...
	public void beforeTestCases(TestSuiteContext context, TestCases testCases, URL modelURL) {
		final TestRunnerContext ctx = (TestRunnerContext) context;
		...
		try {
			...
			if (parseResult.isLeft()) {
				...
				logger.error("Failed to parse DMN file: {} {}", modelURL, failure.message());
				ctx.parserException = new RuntimeException(failure.message());
			} else {
	...
	private TestResult evaluate(
		...
		if (!ctx.dmnModel.decisionsByName().contains(decisionName)) {
			return ignoreTest(
					String.format(
							"No decision found with name '%s'. Parsed decisions: '%s'",
							decisionName, ctx.dmnModel.decisionsByName().keySet()));
		}
	...
	private Object extractDecisionResult(
			...
			if (decisionResult.isLeft()) {
				...
				final DmnEngine.EvalResult evalResult = decisionResult.right().get();

				if (evalResult instanceof DmnEngine.Result) {
					return ((DmnEngine.Result) evalResult).value();
				} else {
					return null;
				}
			}
	...
	private static final class TestRunnerContext implements TestSuiteContext {
